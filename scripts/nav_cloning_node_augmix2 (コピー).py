...
def process_image_from_array(image_array, target_size=None):
    try:
        ...
        # 元のサイズに戻す
        augmented_image_pil = PILImage.fromarray((augmented_image * 255).astype(np.uint8))
        augmented_image_original_size = augmented_image_pil.resize(original_size, PILImage.LANCZOS)

        final_image = np.array(augmented_image_original_size) / 255.0
