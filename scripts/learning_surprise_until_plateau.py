...
MIN_EPOCH = int(os.environ.get("NAV_MIN_EPOCH", 800))
PLATEAU_WINDOW = int(os.environ.get("NAV_PLATEAU_WINDOW", 50))
PLATEAU_REL_DROP = float(os.environ.get("NAV_PLATEAU_REL_DROP", 0.01))
CHECKPOINT_EVERY = int(os.environ.get("NAV_CHECKPOINT_EVERY", 50))
...
def main():
    ...
    best_epoch = 0
    loss_history = []
    model_path = os.path.join(output_model_path, SAVE_MODEL)
    best_model_path = os.path.join(
        output_model_path,
        os.path.splitext(SAVE_MODEL)[0] + "_best.pt",
    )

    print(
        "Early-stop settings: "
        f"max_epoch={EPOCH}, min_epoch={MIN_EPOCH}, "
        f"window={PLATEAU_WINDOW}, rel_drop={PLATEAU_REL_DROP}"
    )
    ...
    for epoch in range(EPOCH):
        ...
        if CHECKPOINT_EVERY > 0 and (epoch + 1) % CHECKPOINT_EVERY == 0:
            checkpoint_path = os.path.join(
                output_model_path,
                os.path.splitext(SAVE_MODEL)[0] + f"_epoch{epoch+1}.pt",
            )
            torch.save(model.state_dict(), checkpoint_path)

        if epoch + 1 >= MIN_EPOCH and len(loss_history) > PLATEAU_WINDOW:
            prev_loss = loss_history[-PLATEAU_WINDOW - 1]
            rel_drop = (prev_loss - avg_loss) / max(abs(prev_loss), 1e-12)
            writer.add_scalar('plateau_rel_drop', rel_drop, epoch)
            if rel_drop < PLATEAU_REL_DROP:
                print(
                    "[EARLY_STOP] "
                    f"epoch={epoch+1}, loss={avg_loss:.6f}, "
                    f"best_loss={best_loss:.6f}@{best_epoch}, "
                    f"last_{PLATEAU_WINDOW}_rel_drop={rel_drop:.6f} "
                    f"< {PLATEAU_REL_DROP}"
                )
                break
    ...
    print(f"[DONE] best model : {best_model_path} loss={best_loss:.6f} epoch={best_epoch}")
