...
使い方:
  python compare_datasets_interactive_fixed.py --src /path/to/src_dir --tgt /path/to/tgt_dir
  python compare_datasets_interactive_fixed.py --src /mnt/data/dataset.pt --tgt /mnt/data/dataset_augmix.pt
"""
...
# 追加: 任意拡張子ローダ
# ---------------------------
def _load_any_path(path):
    """拡張子に応じてロードし to_rgb_float01 へ"""
    lower = path.lower()
    if lower.endswith((".png",".jpg",".jpeg",".bmp",".tif",".tiff")):
        return to_rgb_float01(path)
    if lower.endswith(".npy"):
        arr = np.load(path)          # (H,W,C) or (C,H,W) など想定
        return to_rgb_float01(arr)
    if lower.endswith(".npz"):
        data = np.load(path)
        # 最初の配列を採用（必要ならキー指定へ拡張可）
...
def main():
    ...
    # 0件ガード（早めに気づけるように）
    if len(items_s) == 0 or len(items_t) == 0:
        raise SystemExit(f"No images found: src={len(items_s)} files, tgt={len(items_t)} files. "
                         f"Check subdirectories and extensions (png/jpg/bmp/tif/npy/npz).")
    ...
    if mode == "multi_view":
        ...
        for j, v in enumerate(views):
            ...
            title_texts += ((ax_src.title, ax_tgt.title),)
        ...
        def update(i):
            ...
            for j, v in enumerate(views):
                ...
                title_texts[j*1][0].set_text(f"{v} (src)")
                title_texts[j*1][1].set_text(f"{v} (tgt)\n{metrics_to_str(m)}")
