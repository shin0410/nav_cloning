...
OUTPUT_TIME="_center14_gap_taw_singleop_k3_20260308_0310"
OUT_DIR=""
RUN_UNZIP=1
RUN_SPLIT=1
RUN_TRAIN=1
...
usage() {
  cat <<'EOF'
Usage:
  run_taw_singleop_center14_gap_k3_202603.sh [options]

Options:
  --train-rows N            default: 4000
  --output-time NAME        default: _center14_gap_taw_singleop_k3_20260308_0310
  --out-dir DIR             default: data/<output-time>
  --mag N                   TrivialAugmentWide num-magnitude-bins (default: 31)
  --ops LIST                default: AutoContrast,Brightness,Color,Contrast,Equalize,Identity,Posterize,Sharpness,Solarize
...
Scenario:
  center14_gap + TrivialAugmentWide single-op sweep
  fixed K=3, center time = 20260308_140206
  clean train times = all except 13:00 and 15:00

Flow:
  1) split all 9 datasets into train4000 / test_rest
  2) for each op, augment only center14 train split with TAW single-op K=3
  3) merge (clean K=3 times) + (center14 single-op augmentation)
  4) train one model per op
  5) evaluate all op models on the non-augmented test splits of all 9 times
  6) export MAE tables and line plot
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ops)
      ...
      ;;
    --list-ops)
      "$PYTHON_BIN" "$TAW_PY" --num-magnitude-bins "${2:-$MAG}" --list-ops
      exit 0
      ;;
    --skip-unzip)
      RUN_UNZIP=0
...
for f in "$CFG" "$SPLIT_PY" "$TAW_PY" "$LEARN_PY" "$COMPARE_PY" "$PLOT_OP_PY"; do
...
COMPARE_OUT_DIR="$OUT_DIR/compare_taw_singleop_k3_M${MAG}"
...
run_taw_aug_singleop() {
  local out_img="$1"
  local out_vel="$2"
  local op="$3"
  echo "[RUN] TrivialAugmentWide single-op center14_gap (center=$CENTER_TIME K=$K M=$MAG op=$op)"
...
model_name_for_op() {
  ...
  echo "model_gpu_trivialaugwide_K${K}_M${MAG}_OP${safe_op}_center14_gap_split4000.pt"
}
...
{
  echo "scenario,method,k,magnitude_bins,op_name,clean_times"
  for op in "${OPS[@]}"; do
    echo "center14_gap_singleop,taw,$K,$MAG,$op,$(IFS=,; echo "${CLEAN_TIMES_K3[*]}")"
  done
...
if [[ "$RUN_TRAIN" -eq 1 ]]; then
  for op in "${OPS[@]}"; do
    safe_op="$(op_safe_name "$op")"
    aug_img="TrivialAugmentWide_${TRAIN_IMG_DIR}_center14_K${K}_M${MAG}_OP${safe_op}"
    aug_vel="TrivialAugmentWide_${TRAIN_VEL_DIR}_center14_K${K}_M${MAG}_OP${safe_op}"
    mix_img="mix_taw_center14_gap_K${K}_M${MAG}_OP${safe_op}_img"
    mix_vel="mix_taw_center14_gap_K${K}_M${MAG}_OP${safe_op}_vel"
    model_name="$(model_name_for_op "$op")"

    run_taw_aug_singleop "$aug_img" "$aug_vel" "$op"
    build_merged_dataset "$mix_img" "$mix_vel" "taw_op_${safe_op}" "$aug_img" "$aug_vel" "${CLEAN_TIMES_K3[@]}"
    run_train_model "TrivialAugmentWide center14_gap K=${K} M=${MAG} OP=${op}" "$mix_img" "$mix_vel" "$model_name"
  done
else
...
MAP_CSV="$COMPARE_OUT_DIR/taw_singleop_model_map.csv"
...
echo "[STEP] evaluating center14_gap TAW single-op sweep"
cmp_args=(
  "$COMPARE_PY"
...
if [[ "$RUN_PLOT" -eq 1 ]]; then
  echo "[STEP] plotting center14_gap TAW single-op sweep"
  "$PYTHON_BIN" "$PLOT_OP_PY" \
    --summary_csv "$COMPARE_OUT_DIR/summary_by_time_model.csv" \
    --op_map_csv "$MAP_CSV" \
    --title "MAE over test datasets (center14 gap, TAW single-op, K=${K}, M=${MAG})" \
    --out_dir "$COMPARE_OUT_DIR"
else
  echo "[STEP] skip plot (--skip-plot)"
fi

echo "[DONE] finished center14_gap TAW single-op sweep (K=$K, M=$MAG)"
...
echo "  mae_table_taw_op_compare.csv : $COMPARE_OUT_DIR/mae_table_taw_op_compare.csv"
echo "  summary_taw_op_overall.csv   : $COMPARE_OUT_DIR/summary_taw_op_overall.csv"
echo "  plot                         : $COMPARE_OUT_DIR/mae_taw_op_compare.png"
