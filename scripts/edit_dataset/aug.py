...
def augment_alpha_beta(image):
    """float32 (0〜1) 前提のアルファ・ベータ変換"""
    variants = []
    alpha_beta_pairs = [
        (1.0, 0.0),
        (0.6, 0.0),
    ...
    for alpha, beta in alpha_beta_pairs:
        variants.append(np.clip(alpha * image + beta, 0.0, 1.0))
    return variants
...
def augment_gamma_color_shadow_variants(image):
    ...
    for _ in range(3):
        ...
        img = color_jitter_bgr(img, brightness=0.4, contrast=0.4, saturation=0.4, hue=0.1)
    ...
    for episode in tqdm(episodes, desc="Processing episodes"):
        ...
        for pos in ['center', 'left', 'right']:
            ...
            try:
                ...
                # float32で統一されたimg_bgrに対して拡張処理
                alpha_beta_variants = augment_alpha_beta(img_bgr)

                combined_variants = []
                for alb_img in alpha_beta_variants:
                    gamma_variants = augment_gamma_color_shadow_variants(alb_img)
                    combined_variants.extend(gamma_variants)
